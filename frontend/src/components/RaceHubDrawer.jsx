...
export default function RaceHubDrawer({ open, onClose }) {
  ...
  async function handleAdd() {
    ...
    try {
      ...
      flash('Added');
      toast.success(`Added to ${activeSection}.`, 'Race hub');
      load();
    } catch {
      flash('Add failed');
      toast.error('Could not add item.', 'Race hub');
    }
  }

  async function handleDelete(id) {
    await deleteRaceHubItem(id);
    if (editingId === id) { setEditingId(null); setEditing(null); }
    toast.info('Item deleted.', 'Race hub');
    load();
  }
  ...
  function cancelEdit() { setEditingId(null); setEditing(null); }
  ...
  async function saveEdit() {
    ...
    setEditing(null);
    flash('Saved');
    toast.success('Item saved.', 'Race hub');
    load();
  }
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Racing and media ops"
        aria-hidden={!open}
      >
        ...
        <div className="rh-tabs" role="tablist">
          {SECTIONS.map(s => {
            ...
            return (
              ...
              >
                ...
                {c.attention > 0 && (
                  <span className="rh-tab-attn" title={`${c.attention} need attention`}>!</span>
                )}
              </button>
            );
          })}
            ...
            return (
              <div key={item.id} className={'rh-item status-' + item.status}>
                <button
                  ...
                  aria-label={`Cycle status. Current: ${status.label}`}
                >
                  {status.icon}
                </button>
                ...
                <div className="rh-actions">
                  <button onClick={() => startEdit(item)} title="Edit" aria-label={`Edit ${item.title}`}>✎</button>
                  <button onClick={() => handleDelete(item.id)} title="Delete" aria-label={`Delete ${item.title}`}>×</button>
                </div>
              </div>
            );
          })}
        ...
        <div className="rh-add">
          ...
          <div className="sg">
            ...
            <input
              placeholder={
                activeSection === 'fleet'      ? 'e.g. Front-right tire — replace before next race' :
